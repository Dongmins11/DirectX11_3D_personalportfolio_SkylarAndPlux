...
public:
	ID3D11ShaderResourceView* Get_ResourceView(_uint iIndex = 0) { return m_Textures[iIndex]; }
public:
	virtual HRESULT NativeConstruct_Prototype(const _tchar* pTextureFilePath, _uint iNumTextures);
	virtual HRESULT NativeConstruct(void* pArg);

public:
	HRESULT SetUp_OnShader(const char* pConstructTableName, class CVIBuffer* pVIBuffer, _uint iIndex = 0);
	HRESULT SetUp_OnShader(const char* pConstructTableName, class CModel* pModel, _uint iIndex = 0);
private:
