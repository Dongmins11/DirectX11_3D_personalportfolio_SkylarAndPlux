...
BEGIN(Client)
class CChangeFloorThree final : public CGameObject
{
private:
	explicit CChangeFloorThree(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CChangeFloorThree(const CChangeFloorThree& rhs);
	virtual ~CChangeFloorThree() = default;
public:
...
public:
	static	CChangeFloorThree* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
