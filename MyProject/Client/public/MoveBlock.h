...
BEGIN(Client)
class CMoveBlock final : public CGameObject
{
private:
	explicit CMoveBlock(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CMoveBlock(const CMoveBlock& rhs);
	virtual ~CMoveBlock() = default;
public:
...
public:
	static	CMoveBlock* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
