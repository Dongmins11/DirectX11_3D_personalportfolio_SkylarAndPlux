...
BEGIN(Client)
class ChangeFloor final : public CGameObject
{
private:
	explicit ChangeFloor(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit ChangeFloor(const ChangeFloor& rhs);
	virtual ~ChangeFloor() = default;
public:
...
public:
	static	ChangeFloor* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
