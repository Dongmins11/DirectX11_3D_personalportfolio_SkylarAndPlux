...
BEGIN(Client)
class CDefferde_TimeSphere final : public CGameObject
{
private:
	explicit CDefferde_TimeSphere(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
	explicit CDefferde_TimeSphere(const CDefferde_TimeSphere& rhs);
	virtual ~CDefferde_TimeSphere() = default;
public:
...
public:
	static	CDefferde_TimeSphere* Create(ID3D11Device* pDevice, ID3D11DeviceContext* pDeviceContext);
