#include "stdafx.h"
#include "..\Public\Box_Ui.h"

#include "GameInstance.h"

CBox_Ui::CBox_Ui(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CBox_Ui::CBox_Ui(const CBox_Ui & rhs)
	: CGameObject(rhs)
{
}

HRESULT CBox_Ui::Native_Construct_Prototype()
{
...
HRESULT CBox_Ui::Native_Construct(void* pArg, _uint iMyLevelIndex)
{
	...
	wsprintf(m_szBuffer, TEXT("%d / %d"), m_iCurBox, m_iMaxBox);
	
	return S_OK;
}

_int CBox_Ui::Tick(_double TimeDelta)
{
...
_int CBox_Ui::LateTick(_double TimeDelta)
{
...
HRESULT CBox_Ui::Render()
{
	...
	if (FAILED(pGameInstance->Draw_Font(TEXT("Font_Gulim"), m_szBuffer, _float2(1083, 151), XMVectorSet(1.f, 1.f, 1.f, 1.f), 0.5f)))
		return E_FAIL;
...
HRESULT CBox_Ui::Trans_Render()
{

	return S_OK;
}

HRESULT CBox_Ui::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CBox_Ui::SetUp_ConstantTable()
{
	...
	m_pTextureCom->SetUp_OnShader("g_DiffuseTexture",m_pVIBufferCom, 4);

	return S_OK;
}

HRESULT CBox_Ui::SetUp_ConstantBackTable()
{
	if (nullptr == m_pVIBufferCom)
		return E_FAIL;
	...
	m_pTextureBackCom->SetUp_OnShader("g_DiffuseTexture",m_pVIBufferBackCom, 5);

	return S_OK;
}

CBox_Ui * CBox_Ui::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CBox_Ui*	pInstance = new CBox_Ui(pDevice, pDeviceContext);

	if (FAILED(pInstance->Native_Construct_Prototype()))
	{
		MSGBOX("Failed to Created : CBox_Ui");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CBox_Ui::Clone(void* pArg, _uint iMyLevelIndex)
{
	CBox_Ui*	pInstance = new CBox_Ui(*this);
