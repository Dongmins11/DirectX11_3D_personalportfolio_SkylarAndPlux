#include "stdafx.h"
#include "..\public\MoveBlock.h"
#include "GameInstance.h"
#include "Rock.h"


CMoveBlock::CMoveBlock(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CMoveBlock::CMoveBlock(const CMoveBlock & rhs)
	: CGameObject(rhs)
{
}

HRESULT CMoveBlock::Native_Construct_Prototype()
{
...
HRESULT CMoveBlock::Native_Construct(void* pArg, _uint iMyLevelIndex)
{
	...
	if (nullptr != pArg)
	{
		...
		m_pRopeTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(m_pModelDesc_Client->vPos.x, m_pModelDesc_Client->vPos.y, (m_pModelDesc_Client->vPos.z - 8.5) , m_pModelDesc_Client->vPos.w));
...
_int CMoveBlock::Tick(_double TimeDelta)
{
...
_int CMoveBlock::LateTick(_double TimeDelta)
{
...
HRESULT CMoveBlock::Player_Collision_Check(_double TimeDelta)
{
...
HRESULT CMoveBlock::Rope_Look(_double TimeDelta)
{
...
HRESULT CMoveBlock::Move_State(_double TimeDelta)
{
	...
	if (false == m_bMoveCheck)
	{
		...
		if (false == pTargetSphere->Get_CheckCollision() && true == m_pSphere->Collision_Sphere(pTargetSphere))
		{
			...
			m_pRopeTransformCom->Set_State(CTransform::STATE_POSITION, XMVectorSet(Position.x, Position.y, (Position.z - 8.5f), 1.f));
...
HRESULT CMoveBlock::SetUp_Component_Load()
{
	/* Com_Renderer */
...
HRESULT CMoveBlock::SetUp_Component()
{
	/* Com_Renderer */
	if (FAILED(__super::SetUp_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
		return E_FAIL;
...
HRESULT CMoveBlock::SetUp_ConstantTable()
{
...
CMoveBlock * CMoveBlock::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CMoveBlock* pInstance = new CMoveBlock(pDevice, pDeviceContext);
	if (FAILED(pInstance->Native_Construct_Prototype()))
	{
		MSGBOX("Failed to Create CMoveBlock");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMoveBlock::Clone(void* pArg, _uint iMyLevelIndex)
{
	CMoveBlock* pInstance = new CMoveBlock(*this);
