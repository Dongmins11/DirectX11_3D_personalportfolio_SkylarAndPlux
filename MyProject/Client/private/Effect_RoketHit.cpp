#include "stdafx.h"
#include "..\public\Effect_RoketHit.h"
#include "GameInstance.h"
#include "Rock.h"

CEffect_RoketHit::CEffect_RoketHit(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CEffect_RoketHit::CEffect_RoketHit(const CEffect_RoketHit & rhs)
	: CGameObject(rhs)
{
}

HRESULT CEffect_RoketHit::Native_Construct_Prototype()
{
...
HRESULT CEffect_RoketHit::Native_Construct(void* pArg, _uint iMyLevelIndex)
{
...
_int CEffect_RoketHit::Tick(_double TimeDelta)
{
...
_int CEffect_RoketHit::LateTick(_double TimeDelta)
{
...
HRESULT CEffect_RoketHit::Render()
{
	...
	if (FAILED(m_pModelCom->Render(20)))
		return E_FAIL;

	return S_OK;
}

void CEffect_RoketHit::BillBord_Object()
{
...
_int CEffect_RoketHit::Effect_Change(_double TimeDelta)
{
	if(LEVEL_BOSS_STAGE ==  m_iMyLevel)
		m_dTimeAccFilp += TimeDelta * 2;
	else
		m_dTimeAccFilp += TimeDelta * 10;
...
_int CEffect_RoketHit::Effect_ChangeTwo(_double TimeDelta)
{
	if (LEVEL_BOSS_STAGE == m_iMyLevel)
		m_dTimeAccFilp += TimeDelta * 2;
	else
	...
	if(LEVEL_BOSS_STAGE == m_iMyLevel)
		m_fScale += TimeDelta * 8;
	else
		m_fScale += TimeDelta * 2;
...
HRESULT CEffect_RoketHit::Move_Effect(_double TimeDelta)
{
	_float4 vPos;
...
HRESULT CEffect_RoketHit::SetUp_Component_Load()
{
	/* Com_Renderer */
...
HRESULT CEffect_RoketHit::SetUp_Component()
{
	/* Com_Renderer */
	if (FAILED(__super::SetUp_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
		return E_FAIL;
...
HRESULT CEffect_RoketHit::SetUp_ConstantTable()
{
...
CEffect_RoketHit * CEffect_RoketHit::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CEffect_RoketHit* pInstance = new CEffect_RoketHit(pDevice, pDeviceContext);
	if (FAILED(pInstance->Native_Construct_Prototype()))
	{
		MSGBOX("Failed to Create CEffect_RoketHit");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CEffect_RoketHit::Clone(void* pArg, _uint iMyLevelIndex)
{
	CEffect_RoketHit* pInstance = new CEffect_RoketHit(*this);
