#include "stdafx.h"
#include "..\Public\Magenetic_Ui.h"
#include "Magenetic_Skill.h"
#include "GameInstance.h"

CMagenetic_Ui::CMagenetic_Ui(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	: CGameObject(pDevice, pDeviceContext)
{
}

CMagenetic_Ui::CMagenetic_Ui(const CMagenetic_Ui & rhs)
	: CGameObject(rhs)
{
}

HRESULT CMagenetic_Ui::Native_Construct_Prototype()
{
...
HRESULT CMagenetic_Ui::Native_Construct(void* pArg, _uint iMyLevelIndex)
{
	...
	m_pMagenetic_Skill = pGameInstance->Get_GameObject(m_iMyLevel, L"Clone_Magenetic_Skill");
	if (nullptr == m_pMagenetic_Skill)
	{
		MSGBOX("Failed to MageneticSkill For Magenetic_Ui : NativeConstruct");
		Safe_Release(pGameInstance);
		return E_FAIL;
	}
	Safe_AddRef(m_pMagenetic_Skill);

	Safe_Release(pGameInstance);

	m_fRectColor = _float4(243.f / 255.f, 199.f / 255.f, 60.f / 255.f, 1.f);

	return S_OK;
}

_int CMagenetic_Ui::Tick(_double TimeDelta)
{
...
_int CMagenetic_Ui::LateTick(_double TimeDelta)
{
...
HRESULT CMagenetic_Ui::Render()
{
	...
	m_pVIBufferFrontCom->Render(m_iPassIndex);
...
HRESULT CMagenetic_Ui::Trans_Render()
{
	if (nullptr == m_pMagenetic_Skill)
		return E_FAIL;

	if (true == dynamic_cast<CMagenetic_Skill*>(m_pMagenetic_Skill)->Get_RenderCheck())
	{
		m_iPassIndex = 13;
	}
	else
...
HRESULT CMagenetic_Ui::SetUp_Component()
{
	/* Com_Renderer */
...
HRESULT CMagenetic_Ui::SetUp_ConstantTable()
{
	...
	m_pTextureCom->SetUp_OnShader("g_DiffuseTexture", m_pVIBufferCom, 8);

	return S_OK;
}

HRESULT CMagenetic_Ui::SetUp_ConstantFrontTable()
{
	if (nullptr == m_pVIBufferCom)
		return E_FAIL;
	...
	if(13 == m_iPassIndex)
		m_pVIBufferFrontCom->Set_RawValue("g_RectColor", &m_fRectColor, sizeof(_float4));
...
HRESULT CMagenetic_Ui::SetUp_ConstantBackTable()
{
	if (nullptr == m_pVIBufferCom)
		return E_FAIL;
...
CMagenetic_Ui * CMagenetic_Ui::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CMagenetic_Ui*	pInstance = new CMagenetic_Ui(pDevice, pDeviceContext);

	if (FAILED(pInstance->Native_Construct_Prototype()))
	{
		MSGBOX("Failed to Created : CMagenetic_Ui");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CMagenetic_Ui::Clone(void* pArg, _uint iMyLevelIndex)
{
	CMagenetic_Ui*	pInstance = new CMagenetic_Ui(*this);
...
void CMagenetic_Ui::Free()
{
	__super::Free();

	Safe_Release(m_pMagenetic_Skill);
