#include "stdafx.h"
#include "..\public\SaveCollider_Object.h"
#include "GameInstance.h"
#include "Rock.h"

CSaveCollider_Object::CSaveCollider_Object(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CSaveCollider_Object::CSaveCollider_Object(const CSaveCollider_Object & rhs)
	: CGameObject(rhs)
{
}

HRESULT CSaveCollider_Object::Native_Construct_Prototype()
{
...
HRESULT CSaveCollider_Object::Native_Construct(void* pArg, _uint iMyLevelIndex)
{
...
_int CSaveCollider_Object::Tick(_double TimeDelta)
{
...
_int CSaveCollider_Object::LateTick(_double TimeDelta)
{
...
HRESULT CSaveCollider_Object::Render()
{
...
HRESULT CSaveCollider_Object::Collision_Check()
{
	...
	{
		if (false == m_bSavePointCheck)
		{
			_float4 SavePosition;
			XMStoreFloat4(&SavePosition, m_pTransformCom->Get_State(CTransform::STATE_POSITION));
			dynamic_cast<CRock*>(pPlayerObject)->Set_SavePosition(SavePosition);
			m_bSavePointCheck = true;
		}
	}
...
HRESULT CSaveCollider_Object::Setup_Component_Load()
{
	/* Com_Renderer */
...
HRESULT CSaveCollider_Object::Setup_Component()
{
	/* Com_Renderer */
	if (FAILED(__super::SetUp_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
		return E_FAIL;
...
HRESULT CSaveCollider_Object::Setup_ConstantTable()
{
	return S_OK;
}

CSaveCollider_Object * CSaveCollider_Object::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CSaveCollider_Object*	pInstance = new CSaveCollider_Object(pDevice, pDeviceContext);

	if (FAILED(pInstance->Native_Construct_Prototype()))
	{
		MSGBOX("Failed to Created : CSaveCollider_Object");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CSaveCollider_Object::Clone(void* pArg, _uint iMyLevelIndex)
{
	CSaveCollider_Object*	pInstance = new CSaveCollider_Object(*this);
...
void CSaveCollider_Object::Free()
{
	__super::Free();
