#include "stdafx.h"
#include "..\public\ChuckRock.h"
#include "GameInstance.h"


CChuckRock::CChuckRock(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
	:CGameObject(pDevice, pDeviceContext)
{
}

CChuckRock::CChuckRock(const CChuckRock & rhs)
	:CGameObject(rhs)
{
}

HRESULT CChuckRock::Native_Construct_Prototype()
{
...
HRESULT CChuckRock::Native_Construct(void* pArg, _uint iMyLevelIndex)
{
	...
	if (nullptr != pArg)
	{
		...
		if (1 == m_pModelDesc_Client->viLevel)
		{
...
_int CChuckRock::Tick(_double TimeDelta)
{
...
_int CChuckRock::LateTick(_double TimeDelta)
{
...
HRESULT CChuckRock::SetUp_Component_Load()
{
	/* Com_Renderer */
...
HRESULT CChuckRock::SetUp_Component()
{
	/* Com_Renderer */
	if (FAILED(__super::SetUp_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"), TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
		return E_FAIL;
	...
	/* Com_VIBuffer */
	if (FAILED(__super::SetUp_Component(LEVEL_STATIC, L"ChuckRock0.fbx", TEXT("Com_VIBuffer"), (CComponent**)&m_pModelCom)))
		return E_FAIL;
...
HRESULT CChuckRock::SetUp_ConstantTable()
{
	...
	//if (FAILED(m_pModelCom->Set_TextureValue("g_DepthTexture", pGameInstance->Get_ShaderResourceView(L"Target_FirstDepth"))))
		//return E_FAIL;

	if (FAILED(m_pSourceTextureCom->SetUp_OnShader("g_SourceDiffuseTexture", m_pModelCom,0)))
		return E_FAIL;

	//if (FAILED(m_pSourceTextureCom->SetUp_OnShader("g_SourceDepthDiffuseTexture", m_pModelCom, 1)))
		//return E_FAIL;

	if (FAILED(m_pNomalTextureCom->SetUp_OnShader("g_SoureNomalTexture", m_pModelCom)))
		return E_FAIL;

	if (FAILED(m_pFilterTextureCom->SetUp_OnShader("g_FilterTexture", m_pModelCom,2)))
		return E_FAIL;
...
CChuckRock * CChuckRock::Create(ID3D11Device * pDevice, ID3D11DeviceContext * pDeviceContext)
{
	CChuckRock* pInstance = new CChuckRock(pDevice, pDeviceContext);
	if (FAILED(pInstance->Native_Construct_Prototype()))
	{
		MSGBOX("Failed to Create ChuckRock");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CChuckRock::Clone(void* pArg, _uint iMyLevelIndex)
{
	CChuckRock* pInstance = new CChuckRock(*this);
