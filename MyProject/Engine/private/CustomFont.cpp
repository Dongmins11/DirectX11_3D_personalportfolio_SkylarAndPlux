...
HRESULT CCustomFont::Draw(const _tchar* pText, _float2 vPosition, _fvector vColor,_float fScale)
{
	if (nullptr == m_pSpriteBatch ||
		nullptr == m_pSpriteFont)
		return E_FAIL;

	_float2 Test = _float2(0.f, 0.f);
	m_pSpriteBatch->Begin();

	m_pSpriteFont->DrawString(m_pSpriteBatch, pText, vPosition, vColor,0.f, Test, fScale);
