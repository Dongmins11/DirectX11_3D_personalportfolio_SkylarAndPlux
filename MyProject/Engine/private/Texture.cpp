...
HRESULT CTexture::SetUp_OnShader(const char* pConstructTableName,CVIBuffer * pVIBuffer, _uint iIndex)
{
	if (iIndex >= m_iNumTextures)
		return E_FAIL;

	pVIBuffer->Set_TextureValue(pConstructTableName, m_Textures[iIndex]);

	return S_OK;
}

HRESULT CTexture::SetUp_OnShader(const char * pConstructTableName, CModel * pModel, _uint iIndex)
{
	if (iIndex >= m_iNumTextures)
		return E_FAIL;

	pModel->Set_TextureValue(pConstructTableName, m_Textures[iIndex]);
