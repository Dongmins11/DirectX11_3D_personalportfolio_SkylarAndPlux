...
{
	memcpy(m_vOriginalPoints, pPoints, sizeof(_float3) * POINT_END);
...
_bool CCell::isIn(_fvector vPosition, CCell** ppNeighbor)
{
...
HRESULT CCell::Ready_Debug_Buffer()
{
	...
	m_pVIBuffer = CVIBuffer_LineStrip::Create(m_pDevice, m_pDeviceContext, vPoints, 4);
	if (nullptr == m_pVIBuffer)
		return E_FAIL;
